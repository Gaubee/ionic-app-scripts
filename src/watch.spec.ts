...
describe("watch", () => {
  ...
  describe("prepareWatcher", () => {
    ...
    it("should set replacePathVars when options.ignored is a string", () => {
      ...
      watch.prepareWatcher(context, watcher);
      expect(watcher.options.ignored).toEqual(
        join(process.cwd(), "some", "src", "**", "*.spec.ts")
      );
    });
    ...
    it("should set replacePathVars when options.ignored is an array of strings", () => {
      ...
      watch.prepareWatcher(context, watcher);
      expect((watcher.options.ignored as string[])[0]).toEqual(
        join(process.cwd(), "some", "src", "**", "*.spec.ts")
      );
      expect((watcher.options.ignored as string[])[1]).toEqual(
        join(process.cwd(), "some", "src", "index.html")
      );
    });
    ...
    it("should set replacePathVars when paths is an array", () => {
      ...
      expect(watcher.paths[0]).toEqual(
        join(process.cwd(), "some", "src", "some", "path1")
      );
      expect(watcher.paths[1]).toEqual(
        join(process.cwd(), "some", "src", "some", "path2")
      );
    });
    ...
    it("should set replacePathVars when paths is a string", () => {
      ...
      expect(watcher.paths).toEqual(
        join(process.cwd(), "some", "src", "some", "path")
      );
    });
  ...
  describe("queueOrRunBuildUpdate", () => {
    it("should not queue a build when there isnt an active build", () => {
      ...
      spyOn(watch, "queueOrRunBuildUpdate").and.callThrough();
      spyOn(build, "buildUpdate").and.returnValue(Promise.resolve());
    ...
    it("should queue up changes when a build is active", () => {
      ...
      spyOn(watch, "queueOrRunBuildUpdate").and.callThrough();
      const buildUpdateSpy = spyOn(build, "buildUpdate").and.callFake(
      ...
      const promise = watch.queueOrRunBuildUpdate(
      ...
      expect(watch.queuedChangedFileMap.get(changedFileThree.filePath)).toEqual(
        changedFileThree
      );
      expect(watch.queuedChangedFileMap.get(changedFileFour.filePath)).toEqual(
        changedFileFour
      );
      expect(build.buildUpdate).toHaveBeenCalledTimes(1);
      ...
      expect(watch.queuedChangedFileMap.get(changedFileTwo.filePath)).toEqual(
        changedFileTwo
      );
      expect(watch.queuedChangedFileMap.get(changedFileThree.filePath)).toEqual(
        changedFileThree
      ...
      expect(watch.queuedChangedFileMap.get(changedFileFive.filePath)).toEqual(
        changedFileFive
      );
      expect(watch.queuedChangedFileMap.get(changedFileSix.filePath)).toEqual(
        changedFileSix
      );
      expect(build.buildUpdate).toHaveBeenCalledTimes(1);
      ...
      return promise.then(() => {
        ...
        expect(buildUpdateSpy.calls.first().args[0]).toEqual(
          originalChangedFiles
        );
        ...
        expect(
          buildUpdateSpy.calls
            ...
            .args[0].concat()
            .filter(
              (changedFile: ChangedFile) => changedFile === changedFileTwo
            )[0]
        ...
        expect(
          buildUpdateSpy.calls
            ...
            .args[0].concat()
            .filter(
              (changedFile: ChangedFile) => changedFile === changedFileThree
            )[0]
        ...
        expect(
          buildUpdateSpy.calls
            ...
            .args[0].concat()
            .filter(
              (changedFile: ChangedFile) => changedFile === changedFileFour
            )[0]
        ...
        expect(
          buildUpdateSpy.calls
            ...
            .args[0].concat()
            .filter(
              (changedFile: ChangedFile) => changedFile === changedFileFive
            )[0]
        ...
        expect(
          buildUpdateSpy.calls
            ...
            .args[0].concat()
            .filter(
              (changedFile: ChangedFile) => changedFile === changedFileSix
            )[0]
    ...
    it("should handle multiple queueing and unqueuing events aka advanced test", () => {
      ...
      const thirdPromise = new Promise<void>((resolve, reject) => {
      ...
      return firstPromise
        .then(() => {
          ...
          expect(watch.queuedChangedFileMap.size).toEqual(2);
          expect(
            watch.queuedChangedFileMap.get(changedFileOne.filePath)
          ).toEqual(changedFileOne);
          expect(
            watch.queuedChangedFileMap.get(changedFileThree.filePath)
          ).toEqual(changedFileThree);
        ...
        .then(() => {
          ...
          expect(
            buildUpdateSpy.calls
              ...
              .args[0].concat()
              .filter(
                (changedFile: ChangedFile) => changedFile === changedFileOne
              )[0]
          ).toEqual(changedFileOne);
          expect(
            buildUpdateSpy.calls
          ...
          expect(
            watch.queuedChangedFileMap.get(changedFileFour.filePath)
          ).toEqual(changedFileFour);
          expect(
            watch.queuedChangedFileMap.get(changedFileFive.filePath)
          ).toEqual(changedFileFive);
          expect(
            watch.queuedChangedFileMap.get(changedFileSix.filePath)
          ).toEqual(changedFileSix);
