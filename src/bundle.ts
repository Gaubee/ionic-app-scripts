...
import {
  ...
  webpackUpdate,
  getWebpackConfig,
  getOutputDest as webpackGetOutputDest,
} from "./webpack";

export function bundle(context: BuildContext, configFile?: string) {
  return bundleWorker(context, configFile).catch((err: Error) => {
...
export function bundleUpdate(
  changedFiles: ChangedFile[],
  context: BuildContext
) {
  return webpackUpdate(changedFiles, context).catch((err) => {
...
export function buildJsSourceMaps(context: BuildContext) {
  ...
  return !!(webpackConfig.devtool && (webpackConfig.devtool as string) !== "");
}
