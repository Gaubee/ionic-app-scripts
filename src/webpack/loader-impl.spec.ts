...
describe("webpack loader", () => {
  it("should callback with file and original source map provided", (done: Function) => {
    ...
    spyOn(mockWebpackObject, "cacheable");
    spyOn(mockWebpackObject, "async").and.returnValue(spy);
    spyOn(helpers, "getContext").and.returnValue(mockContext);
    spyOn(helpers, "readAndCacheFile").and.returnValue(
      Promise.resolve(fakeContent)
    );
    spyOn(mockContext.fileCache, "get").and.returnValue({
      path: fakePath,
      content: fakeContent,
    });
  ...
  it("should callback with file and map loaded from file cache", (done: Function) => {
    ...
    const fileCacheSpy = spyOn(mockContext.fileCache, "get").and.returnValue({
      path: fakePath,
      content: fakeContent,
    });
    ...
    const assertFunction = () => {
      ...
      expect(fileCacheSpy.calls.mostRecent().args[0]).toEqual(
        fakePath + ".map"
      );
  ...
  it("should callback with error when can't load file from disk", (done: Function) => {
    ...
    spyOn(mockWebpackObject, "async").and.returnValue(spy);
    spyOn(helpers, "getContext").and.returnValue(mockContext);
    spyOn(mockContext.fileCache, "get").and.returnValue(null);
    spyOn(mockContext.fileCache, "set");
    spyOn(helpers, "readAndCacheFile").and.returnValue(
      Promise.reject(new Error(cantReadFileError))
    );
  ...
  it("should callback with content from disk", (done: Function) => {
    ...
    spyOn(mockWebpackObject, "cacheable");
    spyOn(mockWebpackObject, "async").and.returnValue(callbackSpy);
    spyOn(helpers, "getContext").and.returnValue(mockContext);
    spyOn(mockContext.fileCache, "set");
    const readFileSpy = spyOn(helpers, "readAndCacheFile").and.returnValue(
      Promise.resolve(fakeContent)
    );
    spyOn(mockContext.fileCache, "get").and.returnValue({
      path: fakePath,
