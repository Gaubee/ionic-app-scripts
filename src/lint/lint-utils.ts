...
export function lintFiles(
  ...
  return typeCheck(context, program, linterOptions)
    ...
    .then(() =>
      Promise.all(
        filePaths.map((filePath) => lintFile(linter, config, filePath))
      )
...
export function lintFile(
  ...
  return readFileAsync(filePath).then((fileContents: string) =>
 ...
 */
export function processTypeCheckDiagnostics(
  context: BuildContext,
  ...
  if (tsDiagnostics.length > 0) {
    ...
    const files = removeDuplicateFileNames(
      diagnostics.map((diagnostic) => diagnostic.relFileName)
    );
    const errorMessage = generateErrorMessageForFiles(
      files,
      "The following files failed type checking:"
    );
...
export function generateErrorMessageForFiles(
  failingFiles: string[],
  message?: string
) {
  return `${
    message || "The following files did not pass tslint:"
  }\n${failingFiles.join("\n")}`;
}
...
export function getFileNames(
  ...
  return failures.map((failure) =>
    failure.getFileName().replace(context.rootDir, "").replace(/^\//g, "")
  );
}
